...
try {
  elmPackageJson = fs.readFileSync("elm.json", { encoding: "utf-8" });
} catch (e) {
  console.error(
    "There is no elm-package.json in the directory your are running elm-outdated from."
  );
  process.exit(1);
}

fetch("https://alpha.elm-lang.org/all-packages")
  .then(data => {
    let parsedJson;
